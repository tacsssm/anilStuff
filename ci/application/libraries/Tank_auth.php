...
class Tank_auth
{
	...
	function __construct()
	{
		...
		$this->ci->load->config('redtown', TRUE);
	...
	{
		return $this->ci->config->item('files_root', 'redtown');
	}
	 ...
	 * @return	bool
	 */
	function login($login, $password, $remember, $login_by_username, $login_by_email, $fb_id)
	{
		if ($fb_id) //added  by Myro
			{	
				if (!is_null($user = $this->ci->users->get_user_by_fbid($fb_id))) {	// login ok
				...
				$this->error = array('no_fbid_in_db' => '');
			}	
	
	
	
		else if (((strlen($login) > 0) AND (strlen($password) > 0))) {
	...
	{
		list($email_username, $email_extension) = explode("@", $email);
		return $this->ci->users->check_whitelist($email_extension);
	}
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation, $fbid = NULL)
	{
		if (!$this->ci->users->is_email_available($email)) {
			$this->error = array('email' => 'auth_email_in_use');

		} elseif (!$this->ci->users->is_fbid_available($fbid) && $fbid) {
			$this->error = array('fbid' => 'auth_fbid_in_use');
			...
			if (!is_null($res = $this->ci->users->create_user($data, (!$email_activation && $this->is_email_whitelisted($email))))) {
